...
public class Menu {
    ...
    public static void main(String[] args) {
        ...
        InputStream is = null;
        if (Config.isSupportCurrencies()) {// Only try to load the currencies file if support.currencies is set to true
            if (Config.getCurrenciesSource().equals("file")) {
                template = new FileHook(Config);
                is = template.getCurrenciesInputStream();
            } else if (Config.getCurrenciesSource().equals("webservice")) {
                template = new WebHook(Config);
                is = template.getCurrenciesInputStream();
            } else if (Config.getCurrenciesSource().equals("rest")) {
                template = new RestHook(Config);
                is = template.getCurrenciesInputStream();
            }
        }

        // get currencies from is
...
*/
    ...
    private static Config readConfigFile(String filename) {
        ...
        return new Config(supportCurrencies, currenciesSource, webserviceUrl, restUrl, currencyFile);
    }
}
